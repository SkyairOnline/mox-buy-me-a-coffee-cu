...
def test_can_fund_and_withdraw_live():
    ...
    assert amount_funded == SEND_VALUE, "Amount funded does not match the expected send value."
    coffee.withdraw()
    assert boa.env.get_balance(coffee.address) == 0, "Coffee contract balance should be zero after withdrawal."